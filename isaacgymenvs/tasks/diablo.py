...
class Diablo(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        if cfg["env"]["test"]:
            def on_press(key):
                try:
                    if key.char == 'q':  # Quit simulation on 'q' key press
                        exit()
                    elif key.char == 'w':
                        self.command_x_range = [1.9,2]
                        self.command_yaw_range = [0,0]
                        print('w')
                    elif key.char == 's':
                        self.command_x_range = [-2,-1.9]
                        self.command_yaw_range = [0,0]
                        print('s')
                    elif key.char == 'a':
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        asset_file = "urdf/diablo/urdf/diablo.urdf"
        ...
        diablo_asset = self.gym.load_asset(self.sim, asset_root, asset_file, asset_options)
        self.num_dof = self.gym.get_asset_dof_count(diablo_asset)
        self.num_bodies = self.gym.get_asset_rigid_body_count(diablo_asset)
        ...
        knee_names = [s for s in body_names if "leg" in s]
        print(body_names)
        print(knee_names)
        ...
        for i in range(self.num_envs):
            ...
            diablo_handle = self.gym.create_actor(env_ptr, diablo_asset, start_pose, "diablo", i, 1, 0)
            self.gym.set_actor_dof_properties(env_ptr, diablo_handle, dof_props)
            self.gym.enable_actor_dof_force_sensors(env_ptr, diablo_handle)
            self.envs.append(env_ptr)
            self.diablo_handles.append(diablo_handle)

        for i in range(len(feet_names)):
            self.feet_indices[i] = self.gym.find_actor_rigid_body_handle(self.envs[0], self.diablo_handles[0], feet_names[i])
        for i in range(len(knee_names)):
            self.knee_indices[i] = self.gym.find_actor_rigid_body_handle(self.envs[0], self.diablo_handles[0], knee_names[i])

        self.base_index = self.gym.find_actor_rigid_body_handle(self.envs[0], self.diablo_handles[0], "base_link")
    ...
    def compute_observations(self):
        ...
        self.obs_buf[:] = compute_diablo_observations(  # tensors
                                                        self.root_states,
                                                        self.commands,
                                                        self.dof_pos,
...
#####################################################################
@torch.jit.script
def compute_diablo_observations(root_states,
                                commands,
                                dof_pos,
                                default_dof_pos,
                                dof_vel,
...
def compute_diablo_reward(
    ...
    # print(f'lin_vel_error[0]: {lin_vel_error[0]}')
    # torque penalty
    ...
    # Pitch Penalty
    rew_pitch = torch.square(base_euler[:,1]) * rew_scales["pitch"]
    ...
    # Total Reward
    total_reward = rew_lin_vel_xy + rew_ang_vel_z + rew_torque + rew_height +rew_pitch
